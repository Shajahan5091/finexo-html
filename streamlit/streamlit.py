...
with tab1:
    ...
    with col1:
        ...
        dataframe=session.sql('select distinct("table_schema") from SNOW_MIGRATE_DATABASE.SNOW_MIGRATE_SCHEMA.META_TABLES_STRUCT_SOURCE')
        df_schema=dataframe.to_pandas()
        df_schema.insert(1,"Schema Available On Target",'❌')
        list_schema_target=session.sql("select SCHEMA_NAME from SNOW_MIGRATE_DATABASE.INFORMATION_SCHEMA.SCHEMATA;").collect()
    ...
    with col2:
        ...
        dataframe_table_source=session.sql('select distinct "table_schema"as "Table Schema On Source","table_name" as "Table On Source" ,case when "total_rows" is null then 0 else "total_rows" end as "Table Row Count Source" from SNOW_MIGRATE_DATABASE.SNOW_MIGRATE_SCHEMA.META_TABLES_STRUCT_SOURCE;')
        st.button(":inbox_tray:",help="download Table Report",on_click=Table_report)
        df_table_Source=dataframe_table_source.to_pandas()
        dataframe_table_target=session.sql("select TABLE_SCHEMA,TABLE_NAME,ROW_COUNT from SNOW_MIGRATE_DATABASE.INFORMATION_SCHEMA.TABLES where Table_schema !='INFORMATION_SCHEMA';") 
...
with tab3:
    ...
    def Table_Struct():
        session.write_pandas(source_table,f'Migration_Report_Table_Struct_/time/',database="SNOW_MIGRATE_DATABASE",schema="SNOW_MIGRATE_SCHEMA", auto_create_table=True,overwrite=True,table_type="transient")        
    ...
    source_table_sql=('''select "column_name" as  "Column Available On Source","data_type"as  "Data type On Source"  from SNOW_MIGRATE_DATABASE.SNOW_MIGRATE_SCHEMA.META_COLUMNS_STRUCT_SOURCE where "table_schema"='{schema_name}' and "table_name"='{table_name}' ;''').format(schema_name=schema_name,table_name=table_name)
