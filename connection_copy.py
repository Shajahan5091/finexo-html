...
def setup_logger(log_filename):
    ...
    file_handler = logging.FileHandler('C:/MY/ELAIT/SNOWFLAKE/POC/MIGRATION/LOG_DATA/migration.log')
...
logger = setup_logger('C:/MY/ELAIT/SNOWFLAKE/POC/MIGRATION/LOG_DATA/migration.log')
...
def Migration_report(connection, database, schema):
    try:
        ...
        connection_cursor.execute(f"put file://C:\\MY\\ELAIT\\SNOWFLAKE\\POC\\MIGRATION\\streamlit\\environment.yml @{database}.{schema}.Migration_Report/REPORT_FLD AUTO_COMPRESS=FALSE")
        connection_cursor.execute(f"put file://C:\\MY\\ELAIT\\SNOWFLAKE\\POC\\MIGRATION\\streamlit\\streamlit.py @{database}.{schema}.Migration_Report/REPORT_FLD AUTO_COMPRESS=FALSE")
...
def Check_role_permissions():
    try:
        ...
        log_system_message(logger, """Failed to check custom role permission:
                            {}""".format(str(e)))
        return jsonify({"success": False})
...
def CheckBigqueryDatasetsCreatePermissions():
    ...
    try:
        ...
        if status == "":
            ...
            return jsonify({'success': True, 'message': 'Permissions checked successfully.'})
        else:
            err_msg = str({status})
...
def CheckBucketCreated():
    ...
    try:
        ...
        if success:
            ...
            return jsonify({'success': True, 'message': 'Bucket exists with the given name.'})
        else:
        ...
        log_system_message(logger, """Error occurred:
                            {}""".format(str(e)))
        return jsonify({'success': False, 'error': 'An error occurred while checking bucket existence'}), 500
...
def get_tables():
    ...
    try:
        ...
        logger.info(f"Fetched tables for schema '{schema_name}': {tables}")
        return render_template('schemas_copy.html', show_popup=True, tables=tables, schema=schema_name)
    except Exception as e:
        log_system_message(logger, """An error occurred:
                            {}""".format(str(e)))
        return jsonify({'success': False, 'error': 'An error occurred while fetching tables'}), 500
...
def connect_snowflake():
    try:
        ...
        log_system_message(logger, """An error occurred
                            {}""".format(str(e)))
        return e
        # Handle the error accordingly, maybe return an error page or message
    ...
    try:
        ...
        if isinstance(cursor, str):
            ...
            if row[0] == 10:
                ...
                logger.error("Unexpected result while testing service account connection")
                return jsonify({"success": False, "error": "Unexpected result while testing service account connection"}), 500
    except Exception as e:
...
def grant_access_check():
    try:
        logger.info("Checking if required roles are granted")
        cursor = conn.cursor()
        if isinstance(cursor, str):
            # If cursor is a string, it means an error occurred during Snowflake connection
            error_message = cursor
        ...
        log_system_message(logger, """An error occurred while checking access: 
                            {}""".format(str(e)))
        return jsonify({"success": False, "error": str(e)}), 500

# Simulated function to check access for Warehouse
...
# Simulated function to check access for Database
...
# Simulated function to check access for Role
def check_role_access(cursor):
    try:
        logger.info("Checking access for Role")
...
# Simulated function to check access for User
   ...
   for row in rows:
       ...
       if schema_local in schema_list_user_input:
           ...
           query = """
            ...
            `{}`.{}.INFORMATION_SCHEMA.COLUMNS as c on c.table_name = t.table_name where c.table_name in {} group by c.table_catalog, 
            c.table_name,c.table_schema,t.ddl;
           """
           query_2 = """
            select  c.table_catalog, c.table_schema, c.table_name,  string_agg('$1:'||c.column_name) as table_columns , case when t.ddl like 
...
#Function related to Streamlit app
...
""".format(database=database,schema=schema))
    ...
    # print(results)
    text_file_path = r'C:\MY\ELAIT\SNOWFLAKE\POC\MIGRATION\streamlit\streamlit.py'
