...
def Migration_report(connection,database,schema):
    try:
        ...
        connection_cursor.execute(r"put file://C:\MY\ELAIT\SNOWFLAKE\POC\MIGRATION\streamlit\environment.yml  @{database}.{schema}.Migration_Report/REPORT_FLD AUTO_COMPRESS=FALSE".format(database=database,schema=schema))
        connection_cursor.execute(r"put file://C:\MY\ELAIT\SNOWFLAKE\POC\MIGRATION\streamlit\streamlit.py  @{database}.{schema}.Migration_Report/REPORT_FLD AUTO_COMPRESS=FALSE".format(database=database,schema=schema))
