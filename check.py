   ...
   for row in rows:
       ...
       # FOR EXPORTING DATA
           ...
           for i in range(0,len(df2)):
                ...
                logger.info(f"Preparing to load data for table {table_name} with export type {export_type}")
                ...
                try:
                    ...
                    formatted_columns = format_columns(table_columns)  # Call the function to format the columns
                    copy_command = f"""COPY INTO {database}.{table_schema}.{table_name} FROM (SELECT
                                    {formatted_columns}
                                    FROM @{database}.{schema}.snow_migrate_stage/{table_schema}/{table_name}/{table_name}
                                    (file_format => my_parquet_format))"""
...
def format_columns(column_string, max_line_length=100):
    ...
    for col in columns:
        col = col.strip()
        next_addition = col if not current_line else ', ' + col

        if len(current_line) + len(next_addition) > max_line_length:
            # Add the current line to formatted_columns and start a new one
            formatted_columns.append(current_line)
            current_line = col
        else:
            current_line += next_addition
